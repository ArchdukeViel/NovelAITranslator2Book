...
- Redis: optional (in-memory rate limiter otherwise). The preview lockfile
  does not install the ``redis`` package, and the rate limiter is constructed
  at import time, so the preview environment **must** set
  ``WEB_RATE_LIMITER_BACKEND=memory``; any other value fails the function
  import (fail-closed) at cold start.
...
- Vercel backend install is ``pip install --requirement requirements-vercel.lock
  && pip install --no-deps .``: the fully locked third-party dependency set
  (generated by ``uv export --locked --no-emit-workspace``) plus the local
  ``novelai`` package, without a second dependency resolution.
