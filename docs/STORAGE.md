...
artifacts use `R2GatewayStorage.put_immutable()`. Streamed assets use
`R2GatewayStorage.save_stream()`, which sends a bounded HTTPS request to the
private versioned Worker gateway. The gateway performs the native R2 binding
write, validates fixed metadata, records the provider checksum, and
HEAD-verifies the committed length. A declared length mismatch removes the
just-written object before failing. All R2 writes use bounded timeouts/retries
and explicit missing/permission/conflict classification. The service records
operation count, bytes, latency, checksum, reuse, and error metrics without
recording credentials or content.
