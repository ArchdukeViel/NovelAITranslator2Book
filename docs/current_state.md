...
- **Route uniqueness**: admin behavior is registered only under `/api/admin/*`; the backend `/novels/*`, `/api/novels/*`, and token-based HTML dashboard surfaces are removed. Public browser `/novels/*` pages now reach the frontend catch-all. All OpenAPI operation IDs are unique. [Implemented / Locally validated]
