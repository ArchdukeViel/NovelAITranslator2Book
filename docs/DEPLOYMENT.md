...
- Local, CI, and Docker Node is pinned to 26.7.x in `frontend/.nvmrc`, the CI setup, and production `frontend.Dockerfile`. The Vercel frontend service is an intentional exception: `frontend/package.json` declares `24.x` because Vercel currently rejects Node 26 during build initialization.
