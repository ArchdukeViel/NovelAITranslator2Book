...
- [x] 5. Refactor existing storage consumers
  - [x] 5.1 Update `storage/service.py` — `__init__` accepts backend, uses `_backend.mkdirs` (REQ-1.3)
  - [x] 5.2 Replace direct `open()` / `Path.read_bytes()` in sub-modules with backend calls (REQ-1.3)
  - [x] 5.3 Verify all file paths remain identical in filesystem mode (REQ-5.1)
