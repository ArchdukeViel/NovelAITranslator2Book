...
export function describeApiError(error: unknown) {
  if (error instanceof ApiError) {
    ...
    return {
      ...
      details: error.trace_id
        ? { trace_id: error.trace_id, details: error.details }
        : error.details,
    };
  }
  if (error instanceof Error) {
    return {
      title: error.message,
      explanation:
        "The browser received an application error while running this action.",
      details: undefined,
    };
  }
  return {
    title: "Unknown error",
    explanation:
      "The browser received an unknown error while running this action.",
    details: error,
  };
}
...
export function activityProgress(activity: ActivityRecord): JobProgress {
  ...
  const providerError = metadata.provider_error
    ? [metadata.provider_error]
    : [];
  const progressWarnings = Array.isArray(progress.warnings)
    ? progress.warnings
    : [];
  const metadataWarnings = Array.isArray(metadata.warnings)
    ? metadata.warnings
    : [];
  const progressModelStates = Array.isArray(progress.model_states)
    ? (progress.model_states as ModelState[])
    : [];
  const metadataModelStates = Array.isArray(metadata.model_states)
    ? (metadata.model_states as ModelState[])
    : [];
  return {
    status: activity.status,
    provider_key:
      activity.provider_key ??
      payloadText(progress.provider_key) ??
      payloadText(metadata.provider_key),
    provider_model:
      activity.provider_model ??
      payloadText(progress.provider_model) ??
      payloadText(metadata.provider_model),
    current_stage:
      activity.current_stage ??
      payloadText(progress.current_stage) ??
      payloadText(metadata.current_stage),
    current_label:
      activity.current_label ??
      payloadText(progress.current_label) ??
      payloadText(metadata.current_label),
    completed:
      activity.completed ??
      progressNumber(progress.completed) ??
      progressNumber(metadata.completed),
    total:
      activity.total ??
      progressNumber(progress.total) ??
      progressNumber(metadata.total),
    paused_reason:
      activity.paused_reason ??
      payloadText(progress.paused_reason) ??
      payloadText(metadata.paused_reason),
    resume_after:
      activity.resume_after ??
      payloadText(progress.resume_after) ??
      payloadText(metadata.resume_after),
    selection_reason:
      payloadText(progress.selection_reason) ??
      payloadText(metadata.selection_reason),
    errors:
      activity.errors ??
      (progressErrors.length
        ? progressErrors
        : [...metadataErrors, ...providerError]),
    warnings:
      activity.warnings ??
      (progressWarnings.length ? progressWarnings : metadataWarnings),
    model_states:
      activity.model_states ??
      (progressModelStates.length ? progressModelStates : metadataModelStates),
  };
}

export function activityProgressLabel(activity: ActivityRecord) {
  const progress = activityProgress(activity);
  const parts = [progress.current_label, progress.current_stage].filter(
    Boolean,
  );
  if (
...
export const api = {
  ...
  novel: (novelId: string) =>
    apiFetch<NovelMetadata>(`/admin/novels/${encodeURIComponent(novelId)}`),
  deleteNovel: (novelId: string) =>
    apiFetch<void>(`/admin/novels/${encodeURIComponent(novelId)}`, {
      method: "DELETE",
    }),
  publishNovel: (novelId: string) =>
    apiFetch<NovelPublicationSummary>(
      `/admin/novels/${encodeURIComponent(novelId)}/publish`,
      {
        method: "POST",
      },
    ),
  unpublishNovel: (novelId: string) =>
    apiFetch<NovelPublicationSummary>(
      `/admin/novels/${encodeURIComponent(novelId)}/unpublish`,
      {
        method: "POST",
      },
    ),
  ...
  chapter: (novelId: string, chapterId: string) =>
    apiFetch<ChapterDetail>(
      `/admin/novels/${encodeURIComponent(novelId)}/chapters/${encodeURIComponent(chapterId)}`,
    ),
  ...
  translationEditHistory: (novelId: string, chapterId: string) =>
    apiFetch<{
      novel_id: string;
      chapter_id: string;
      history: TranslationEditHistory[];
    }>(
  ...
  activity: (
    ...
    return apiFetch<{ activity: ActivityRecord[] }>(
      `/admin/activity?${search.toString()}`,
    );
  },
  activityItem: (activityId: string) =>
  ...
  runActivity: (activityId: string) =>
    apiFetch<ActivityRecord>(
      `/admin/activity/${encodeURIComponent(activityId)}/run`,
      {
        method: "POST",
      },
    ),
  retryActivity: (activityId: string) =>
    apiFetch<ActivityRecord>(
      `/admin/activity/${encodeURIComponent(activityId)}/retry`,
      {
        method: "POST",
      },
    ),
  sourceHealth: () =>
    apiFetch<{ sources: SourceHealth[] }>("/admin/activity/source-health"),
  providerApiKeyStatus: (provider = "gemini") =>
    apiFetch<ProviderApiKeyStatus>(
      `/admin/provider-api-key/${encodeURIComponent(provider)}`,
    ),
  setProviderApiKey: (
    payload: ProviderApiKeyValidationPayload & {
      api_key: string;
  ...
  runtimeState: () =>
    apiFetch<{ items: RuntimeStateItem[] }>("/admin/runtime-state"),
  clearRuntimeState: (key: string) =>
    apiFetch<RuntimeStateItem>(
      `/admin/runtime-state/${encodeURIComponent(key)}`,
      {
        method: "DELETE",
      },
    ),
  ...
  workerStart: () =>
    apiFetch<WorkerStatus>("/admin/worker/start", { method: "POST" }),
  workerStop: () =>
    apiFetch<WorkerStatus>("/admin/worker/stop", { method: "POST" }),
  workerRunOnce: () =>
    apiFetch<{ activity: ActivityRecord | null; worker: WorkerStatus }>(
      "/admin/worker/run-once",
      { method: "POST" },
    ),
  schedulerHealth: () =>
    apiFetch<SchedulerHealthResponse>("/admin/translation/scheduler-health"),
  maintenanceStatus: () =>
    apiFetch<MaintenanceStatusResponse>("/admin/maintenance/status"),
  requests: () =>
    apiFetch<{ requests: NovelRequestRecord[] }>("/admin/requests?limit=50"),
  adminReviews: (params?: {
    status?: string;
    page?: number;
    page_size?: number;
  ...
  updateRequestStatus: (
    requestId: string,
    payload: { status: string; reviewed_by?: string; notes?: string },
  ) =>
    apiFetch<NovelRequestRecord>(
  ...
  preliminaryCrawl: (
    ...
    apiFetch<PreliminaryCrawlResult>(
      `/admin/novels/${encodeURIComponent(novelId)}/preliminary-crawl`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      },
  ...
  importNow: (
    novelId: string,
    payload: { source_url: string; max_units?: number | null },
  ) =>
    apiFetch<{
      novel_id: string;
      source_url: string;
  ...
  getTaxonomy: (novelId: string) =>
    apiFetch<NovelTaxonomyResponse>(
      `/admin/novels/${encodeURIComponent(novelId)}/taxonomy`,
    ),
...
type AdminApi = {
  listTakedowns: (
    status?: string,
  ) => Promise<import("./api-types").TakedownListResponse>;
  reviewTakedown: (
    requestId: number,
    payload: { status: string; reviewer_notes?: string },
  ...
  listUsers: (
    filters?: import("./api-types").UserListFilters,
  ) => Promise<import("./api-types").UserListResponse>;
  getUser: (userId: number) => Promise<import("./api-types").UserDetail>;
  updateUserActive: (
    userId: number,
    payload: import("./api-types").ActiveUpdatePayload,
  ) => Promise<import("./api-types").UserDetail>;
  updateUserRole: (
    userId: number,
    payload: import("./api-types").RoleUpdatePayload,
  ) => Promise<import("./api-types").UserDetail>;
  revokeUserSessions: (
    userId: number,
    payload: import("./api-types").RevokeSessionsPayload,
  ) => Promise<import("./api-types").UserDetail>;
  // ── Glossary ─────────────────────────────────────────────────────────
  listGlossaryEntries: (
    novelId: RouteId,
    filters?: GlossaryEntryListFilters,
  ) => Promise<GlossaryEntry[]>;
  createGlossaryEntry: (
    novelId: RouteId,
    payload: GlossaryEntryCreatePayload,
  ) => Promise<GlossaryEntry>;
  previewGlossaryCandidateImport: (
    novelId: RouteId,
    payload?: GlossaryCandidateImportRequest,
  ) => Promise<GlossaryCandidateImportResult>;
  applyGlossaryCandidateImport: (
    novelId: RouteId,
    payload?: GlossaryCandidateImportRequest,
  ) => Promise<GlossaryCandidateImportResult>;
  previewGlossaryProviderCandidates: (
    novelId: RouteId,
    payload?: GlossaryProviderCandidateRequest,
  ) => Promise<GlossaryProviderCandidateResult>;
  applyGlossaryProviderCandidates: (
    novelId: RouteId,
    payload?: GlossaryProviderCandidateRequest,
  ) => Promise<GlossaryProviderCandidateResult>;
  getGlossaryEntry: (
  ...
  updateGlossaryEntry: (
    novelId: RouteId,
    entryId: RouteId,
    payload: GlossaryEntryUpdatePayload,
  ) => Promise<GlossaryEntry>;
  changeGlossaryEntryStatus: (
    novelId: RouteId,
    entryId: RouteId,
    payload: GlossaryEntryStatusPayload,
  ) => Promise<GlossaryEntry>;
  lockGlossaryEntry: (
    novelId: RouteId,
    entryId: RouteId,
    payload?: GlossaryDecisionPayload,
  ) => Promise<GlossaryEntry>;
  unlockGlossaryEntry: (
    novelId: RouteId,
    entryId: RouteId,
  ...
  addGlossaryAlias: (
    novelId: RouteId,
    entryId: RouteId,
    payload: GlossaryAliasCreatePayload,
  ) => Promise<GlossaryAlias>;
  updateGlossaryAlias: (
    novelId: RouteId,
    aliasId: RouteId,
    payload: GlossaryAliasUpdatePayload,
  ) => Promise<GlossaryAlias>;
  deprecateGlossaryAlias: (
    novelId: RouteId,
    aliasId: RouteId,
    payload?: GlossaryDecisionPayload,
  ) => Promise<GlossaryAlias>;
  listGlossaryProvenanceForEntry: (
    novelId: RouteId,
    entryId: RouteId,
  ) => Promise<GlossaryProvenance[]>;
  listGlossaryProvenanceForNovel: (
    novelId: RouteId,
  ) => Promise<GlossaryProvenance[]>;
  addGlossaryProvenance: (
    novelId: RouteId,
    entryId: RouteId,
    payload: GlossaryProvenanceCreatePayload,
  ) => Promise<GlossaryProvenance>;
  listGlossaryDecisionEvents: (
    novelId: RouteId,
    entryId?: RouteId,
  ) => Promise<GlossaryDecisionEvent[]>;
  listGlossaryQaFindings: (
    novelId: RouteId,
    filters?: GlossaryQaFindingListFilters,
  ) => Promise<GlossaryQaFinding[]>;
  createGlossaryQaFinding: (
    novelId: RouteId,
    payload: GlossaryQaFindingCreatePayload,
  ) => Promise<GlossaryQaFinding>;
  updateGlossaryQaFindingStatus: (
    novelId: RouteId,
    findingId: RouteId,
    payload: GlossaryQaFindingStatusPayload,
  ) => Promise<GlossaryQaFinding>;
  transitionGlossaryStatus: (
    novelId: RouteId,
    payload: GlossaryStatusTransitionPayload,
  ) => Promise<GlossaryStatusTransitionResult>;
  batchApproveGlossaryCandidates: (
    novelId: RouteId,
    payload?: GlossaryDecisionPayload,
  ) => Promise<GlossaryBatchApproveResult>;
  ...
  cancelOnboarding: (
    novelId: string,
  ) => Promise<{ novel_id: string; onboarding_status: string }>;
  librarySummary: (params?: {
    refresh?: boolean;
  }) => Promise<import("./api-types").LibrarySummaryResponse>;
  ...
  getAuditEvent: (
    auditId: number,
  ) => Promise<import("./api-types").AuditEventDetail>;
  listProviderCredentialRows: () => Promise<
    import("./api-types").ProviderCredentialListResponse
  >;
...
export const adminApi: AdminApi = {
  ...
  revokeUserSessions: (
    ...
    request<import("./api-types").UserDetail>(
  ...
  listGlossaryEntries: (
    ...
    if (typeof filters.public_visible === "boolean")
      search.set("public_visible", String(filters.public_visible));
    return request<GlossaryEntry[]>(
      appendQuery(`/admin/novels/${routeId(novelId)}/glossary`, search),
    );
  },
  createGlossaryEntry: (
    novelId: RouteId,
  ...
  updateGlossaryAlias: (
    ...
    request<GlossaryAlias>(
      `/admin/novels/${routeId(novelId)}/glossary/aliases/${routeId(aliasId)}`,
      {
        method: "PATCH",
        body: JSON.stringify(payload),
      },
  ...
  deprecateGlossaryAlias: (
    ...
    request<GlossaryAlias>(
      `/admin/novels/${routeId(novelId)}/glossary/aliases/${routeId(aliasId)}/deprecate`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      },
  ...
  listGlossaryProvenanceForEntry: (novelId: RouteId, entryId: RouteId) =>
    request<GlossaryProvenance[]>(
      `${glossaryEntryPath(novelId, entryId)}/provenance`,
    ),
  listGlossaryProvenanceForNovel: (novelId: RouteId) =>
    request<GlossaryProvenance[]>(
      `/admin/novels/${routeId(novelId)}/glossary/provenance`,
    ),
  addGlossaryProvenance: (
    novelId: RouteId,
    entryId: RouteId,
  ...
  listGlossaryQaFindings: (
    ...
    const search = new URLSearchParams();
    if (typeof filters.chapter_id === "number")
      search.set("chapter_id", String(filters.chapter_id));
    if (filters.status) search.set("status", filters.status);
    return request<GlossaryQaFinding[]>(
      appendQuery(
        `/admin/novels/${routeId(novelId)}/glossary/qa-findings`,
        search,
      ),
    );
  },
  ...
  updateGlossaryQaFindingStatus: (
    ...
    request<GlossaryQaFinding>(
      `/admin/novels/${routeId(novelId)}/glossary/qa-findings/${routeId(findingId)}/status`,
      {
        method: "PATCH",
        body: JSON.stringify(payload),
      },
  ...
  transitionGlossaryStatus: (
    ...
    request<GlossaryStatusTransitionResult>(
      `/admin/novels/${routeId(novelId)}/glossary-status`,
      {
        method: "PATCH",
        body: JSON.stringify(payload),
      },
  ...
  batchApproveGlossaryCandidates: (
    ...
    request<GlossaryBatchApproveResult>(
      `/admin/novels/${routeId(novelId)}/glossary/batch-approve`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      },
  ...
  providerCredential: (provider?: string) =>
    request<import("./api-types").ProviderCredential>(
      `/admin/providers/${provider ?? "gemini"}`,
    ),
  ...
  listAuditEvents: (
    ...
    if (typeof filters.actor_user_id === "number")
    ...
    if (filters.correlation_id)
      search.set("correlation_id", filters.correlation_id);
  ...
  getAuditEvent: (auditId: number) =>
    request<import("./api-types").AuditEventDetail>(`/admin/audit/${auditId}`),
