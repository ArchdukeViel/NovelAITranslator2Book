...
const mocks = vi.hoisted(() => ({
  ...
  removeMutations: new Map<
    string,
    { isPending: boolean; mutate: ReturnType<typeof vi.fn> }
  >(),
...
const defaultLibraryData: LibraryItem[] = [
  ...
  {
    slug: "plan-novel",
    status: "plan" as LibraryItem["status"],
    added_at: "2025-01-04T00:00:00Z",
  },
  ...
  {
    slug: "odd-novel",
    status: "odd" as LibraryItem["status"],
    added_at: "2025-01-06T00:00:00Z",
  },
];
...
describe("Library page groups", () => {
  it("renders five named group regions and routes each item to the right one", async () => {
    ...
    expect(
      within(groupSection("Plan to read")).getByText(
    ...
    expect(
      within(groupSection("Completed")).getByText("beta-novel"),
    ).toBeInTheDocument();
    expect(
      within(groupSection("Dropped")).getByText("gamma-novel"),
    ).toBeInTheDocument();
  ...
  it("searches by slug and empties non-matching groups", async () => {
    ...
    await user.type(
      screen.getByRole("searchbox", { name: "Search by slug" }),
      "alpha",
    );
    ...
    for (const title of ["Plan to read", "Completed", "Dropped", "Unknown"]) {
      expect(
        within(groupSection(title)).getByText("No novels in this group yet."),
      ).toBeInTheDocument();
    }
  ...
  it("changes item order within a group when sort changes", async () => {
    ...
    expect(listPresentationOrder(reading)).toEqual([
      "alpha-novel",
      "zeta-novel",
    ]);
  ...
  it("defaults to list presentation on mobile", async () => {
    ...
    expect(
      reading.querySelector('div[class*="sm:grid-cols-2"]'),
  ...
  it("shows empty library CTA linking to /browse-novels", async () => {
    ...
    expect(screen.getByRole("link", { name: "Browse novels" })).toHaveAttribute(
      "href",
      "/browse-novels",
    );
    expect(
  ...
  it("calls useRemoveFromLibrary mutate when Remove clicked", async () => {
    ...
    await user.click(
      within(reading).getByRole("button", {
        name: "Remove zeta-novel from library",
      }),
    );
  ...
  it("shows session loading state while auth is pending", async () => {
    ...
    expect(
      screen.queryByText(/Sign in to save novels/),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("shows error state when library fails", async () => {
    ...
    expect(
      screen.getByText("Could not load your library."),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Try refreshing the page, or return to browse."),
    ).toBeInTheDocument();
  });

  it("disables Remove button while its mutation is pending", async () => {
    mocks.removeMutations.set("zeta-novel", {
      isPending: true,
      mutate: vi.fn(),
    });
    await renderPage();
    ...
    expect(
      within(groupSection("Reading")).getByRole("button", {
        name: "Remove zeta-novel from library",
      }),
    ).toBeDisabled();
  });
