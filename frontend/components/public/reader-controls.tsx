    ...
    <>
      ...
      {open && (
        <section role="dialog" aria-modal="true" aria-label="Reading settings" className="fixed inset-x-3 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-50 mx-auto max-w-md rounded-xl border border-border bg-background p-5 shadow-2xl">
          ...
          <fieldset className="mt-5"><legend className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Text width</legend><div className="mt-2 grid grid-cols-3 gap-2">{WIDTHS.map((item) => <button key={item.value} type="button" aria-label={`${item.label} ${item.width}`} aria-pressed={width === item.value} onClick={() => setWidth(item.value)} className={`rounded-md border px-2 py-2 text-xs ${width === item.value ? "border-primary bg-primary/10" : "border-border"}`}><span className="block font-medium">{item.label}</span><span className="text-muted-foreground">{item.width}</span></button>)}</div></fieldset>
